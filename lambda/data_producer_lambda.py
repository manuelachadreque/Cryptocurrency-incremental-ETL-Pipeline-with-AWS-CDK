...
DICT_KEYS=[
    ("1. From_Currency_Code","from_currency_code"),
    ("2. From_Currency_Name","from_currency_name"),
    ("3. To_Currency_Code","to_currency_code"),
    ("4. To_Currency_Name","to_currency_name"),
...
#Rename keys
def rename_dick_key(dct:dict, old_name:str, new_name:str) ->None:
    dct[new_name] = dct.pop(old_name)

def rename_dict_keys(dct:dict, keys:list):
    for key in keys:
        rename_dick_key(dct, key[0], key[1])
...
def handle_intraday_request(from_currency, to_currency):
    ...
    partitionKey=create_partition_from_date(data["last_refreshed"])
